...
const masterCode_1 = require("../../configs/constants/masterCode");
const codeValidation = {
    async utoken(req, res, next) {
        ...
        try {
            jsonwebtoken_1.default.verify(tocompare, process.env.ACCESS_TOKEN);
        }
        catch (err) {
        ...
        await Users_1.default.findOne({
            ...
            if (result) {
                ...
                await (0, redisConfig_1.getCache)(`new-user:${email.trim().toLowerCase()}`)
                    ...
                    if (tocompare === code || tocompare === masterCode_1.masterCode)
                        return next();
                    else {
