...
import { masterCode } from '../../configs/constants/masterCode';
...
const codeValidation = {
    async utoken(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            jwt.verify(tocompare, process.env.ACCESS_TOKEN as string);
        } catch (err) {
            error(err)
                ...
                if (result) {
                    ...
                    await getCache(`new-user:${email.trim().toLowerCase()}`)
                        .then((code:any) => {
                            ...
                            if (tocompare === code || tocompare === masterCode) return next();
                            else {
